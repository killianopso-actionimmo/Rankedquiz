...
export function GameHeader({ title }: { title: string }) {
  return (
    <header className="flex items-center gap-3 px-4 pb-2 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6">
      <Link
        href="/"
        className="btn-tap flex h-11 w-11 items-center justify-center rounded-full border border-line bg-background-card text-ink shadow-subtle hover:border-primary active:scale-95"
